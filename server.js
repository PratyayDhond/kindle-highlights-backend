...
function validateEmailConfig() {
  ...
  if (!process.env.EMAIL_USER) {
    issues.push('EMAIL_USER environment variable not set');
  }
  
  if (!process.env.EMAIL_PASSWORD) {
    issues.push('EMAIL_PASSWORD environment variable not set');
  } else if (process.env.EMAIL_PASSWORD.length !== 16) {
    issues.push('EMAIL_PASSWORD should be 16-character Gmail App Password');
  }
...
app.post('/admin/test-email-debug', authenticate, async (req, res) => {
  try {
    ...
    const testMail = {
      ...
      text: `Test email sent at ${new Date()} from Render`,
      html: `<h3>🧪 Email Test</h3><p>Sent at: <strong>${new Date()}</strong></p><p>From: Render hosting</p>`
    };

    const result = await sendEmailWithRetry(testMail, 3);
